...
class BacktestEngine:
    ...
    def __init__(self, initial_capital: float = 10000.0, commission: float = 0.001, 
                 min_order_value: float = 1.0, data_dir: str = "data/processed", 
    ...
    def run_backtest(self, strategy: BaseStrategy, data: pd.DataFrame, 
        ...
        self.logger.info(f"Running backtest for {symbol} with {len(data)} data points")
        
        try:
            # Run the actual backtest
            result = self._run_backtest_engine(strategy, data, symbol)
            
            # Save results
            self._save_results(result, strategy.get_strategy_name(), symbol)
    ...
    def _run_backtest_engine(self, strategy: BaseStrategy, data: pd.DataFrame, 
                           symbol: str = "UNKNOWN") -> BacktestResult:
        """
        Core backtest engine logic.
        """
        # Comprehensive input validation
        self._validate_inputs(strategy, data, symbol)
        
        self.logger.info(f"Starting backtest for {symbol} with {len(data)} data points")
                ...
                if hasattr(strategy, 'risk_manager') and strategy.risk_manager is not None:
                    ...
                    if should_close:
                        ...
                        if stop_trade:
                            ...
                            self.logger.info(f"STOP LOSS: {stop_trade.quantity:.4f} shares at ${stop_trade.price:.2f} - {reason}")
            ...
            if not stop_loss_triggered:
                if signal == 1 and cash > 0:  # Buy signal
                    ...
                    if trade:
                        ...
                        self.logger.info(f"BUY: {trade.quantity:.4f} shares at ${trade.price:.2f} "
                                       f"(Value: ${trade.value:.2f}, Commission: ${commission_cost:.2f}, Cash: ${cash:.2f})")
                        if current_stop_loss:
                            self.logger.info(f"Stop loss set at ${current_stop_loss:.2f}")
                    ...
                    if requested_sell_quantity > max_sell_quantity:
                        self.logger.warning(f"Sell quantity {requested_sell_quantity:.4f} exceeds position {max_sell_quantity:.4f}")
                    ...
                    if trade:
                        ...
                        self.logger.info(f"SELL: {trade.quantity:.4f} shares at ${trade.price:.2f} "
                                       f"(Value: ${trade.value:.2f}, Commission: ${commission_cost:.2f}, Cash: ${cash:.2f})")
    ...
    def _validate_inputs(self, strategy: BaseStrategy, data: pd.DataFrame, symbol: str) -> None:
        ...
        if not hasattr(strategy, 'generate_signals'):
            raise ValueError("Strategy must have generate_signals method")
        ...
        self.logger.info(f"Input validation passed for {symbol}")
